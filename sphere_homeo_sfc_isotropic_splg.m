function [M, u, v, T] = sphere_homeo_sfc_isotropic_splg(fctn_x, fctn_y, fctn_z, range_u, range_v, option_random_sampling)
...
% 
% [M1, u, v] = sphere_homeo_sfc_isotropic_splg(fctn_x, fctn_y, fctn_z, range_u, range_v);
% [M2, u, v, T] = sphere_homeo_sfc_isotropic_splg(fctn_x, fctn_y, fctn_z, range_u, range_v, 0);
...
if (size(range_u,2) < 3 || range_u(1,3) < 1)
    range_u = cat(2, range_u, 60);
    warning('range_u : missing or incorrect value for the number of samples ; set to 60 by default');
else
...
assert(u_max >= u_min, 'range_u : must be u_min <= u_max.');
...
assert(v_max >= v_min, 'range_v : must be v_min <= v_max.');

% Surface sampling
[~, u_step] = meshgrid(linspace(-1,1,nb_samples_v), linspace(-1,1,nb_samples_u));
u_rand = 2*(rand(nb_samples_u, nb_samples_v) - 0.5);
